(async function() {
  ...
  // Warning: Running this in a single tab requires the tab to stay open.
  // Because page navigation clears the console, we'll open them in new tabs one-by-one.
  
  for (const url of urls) {
    console.log(`Processing: ${url}`);
    const newTab = window.open(url, '_blank');
    
    // Wait for the tab to load and the LinkedIn 'boot-complete' class to appear
    await new Promise(resolve => {
      const checkLoad = setInterval(() => {
        try {
          if (newTab.document.body.classList.contains('boot-complete')) {
            clearInterval(checkLoad);
            const fileName = url.split('/').filter(Boolean).pop() + ".html";
            download(newTab.document.documentElement.outerHTML, fileName);
            newTab.close();
            resolve();
          }
        } catch (e) {
          // Ignore cross-origin errors if they happen
        }
      }, 3000); // Check every 3 seconds
    });
    
    // Safety delay to prevent LinkedIn from throttling you
